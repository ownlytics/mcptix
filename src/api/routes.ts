...
export function setupRoutes(app: express.Application, ticketQueries: TicketQueries): void {
  ...
  app.put('/api/tickets/:id', validateRequest(validateUpdateTicket), (req: Request, res: Response, next: NextFunction) => {
    try {
      const { title, description, priority, status, complexity_metadata } = req.body;
  ...
  app.post('/api/tickets/:id/comments', validateRequest(validateCreateComment), (req: Request, res: Response, next: NextFunction) => {
    try {
