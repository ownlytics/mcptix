        ...
        inputSchema: {
          ...
          properties: {
            ...
            search: {
              type: 'string',
              description:
                'Text to search for (or regex pattern if useRegex is true). Use this to target specific portions of text without rewriting the entire field.',
            },
            replace: {
              type: 'string',
              description:
                'Replacement text (can include regex capturing groups like $1 if useRegex is true). This will replace only the matched text, preserving the rest of the field content.',
            },
            useRegex: {
              type: 'boolean',
              description:
                'Use regular expressions for search and replace. Enables powerful pattern matching for complex replacements in code blocks, markdown, etc.',
              default: false,
            },
            caseSensitive: {
              type: 'boolean',
...
function handleEditField(ticketQueries: TicketQueries, args: any) {
