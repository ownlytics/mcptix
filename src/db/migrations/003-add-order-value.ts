...
const migration: Migration = {
  ...
  up: (db: Database.Database): void => {
    ...
    if (!hasOrderValue) {
      ...
      Logger.info('Migration', 'Added order_value column to tickets table');
      ...
      Logger.info('Migration', 'Populated order_value for existing tickets');
    } else {
      Logger.info('Migration', 'order_value column already exists');
    }
  },
  ...
  down: (db: Database.Database): void => {
    ...
    if (supportsDropColumn) {
      ...
      Logger.info('Migration', `Removed order_value column using direct DROP COLUMN (SQLite ${sqliteVersion})`);
    } else {
      ...
      Logger.info('Migration', 'Removed order_value column from tickets table using recreation method');
    }
  },
};
